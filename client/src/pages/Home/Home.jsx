...
const Home = () => {
  ...
  const updateIsPinned=async(taskData)=>{
    ...
    try{
        const response=await axiosInstance.put("/update-note-pinned/"+taskId,{
...
const handlePriorityFilterChange = (event) => {
  ...
  getFilteredTasks(statusFilter, event.target.value); 
};
  ...
  return (
    ...
    <div className="flex flex-col sm:flex-row justify-between items-center mb-4 space-y-4 sm:space-y-0 sm:space-x-4">
                <div>
                    ...
                    <select id="statusFilter" value={statusFilter} onChange={handleFilterChange}  className="ml-2 border border-gray-300 rounded-md p-2 focus:outline-none focus:ring-2 focus:ring-blue-500">
                ...
                <div>
                    <label htmlFor="priorityFilter" className='font-semibold'>Filter by priority:</label>
                    <select id="priorityFilter" value={priorityFilter} onChange={handlePriorityFilterChange}  className="ml-2 border border-gray-300 rounded-md p-2 focus:outline-none focus:ring-2 focus:ring-blue-500">
                        <option value="">All</option>
                ...
                <div>
                        <label htmlFor="sortOrder" className='font-semibold'>Sort by Due Date:</label>
                        <select id="sortOrder" value={sortOrder} onChange={(e) => setSortOrder(e.target.value)}  className="ml-2 border border-gray-300 rounded-md p-2 focus:outline-none focus:ring-2 focus:ring-blue-500">
      ...
      {sortedTasks().length >0 ?<div className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 mt-8'>
...
title={item.title}
 ...
 />
    ...
    </div>:<EmptyCard imgSrc={isSearch?noDataImg:addtask} message={isSearch?`Oops! No Tasks found matching your search.`:`Start Creating your first task! Click the 'Add' Button to jot down your tasks,reminders.Lets Get Started!`}/>}
    </div>
   <button className='w-16 h-16 flex items-center justify-center rounded-full bg-primary hover:bg-blue-600 fixed right-4 bottom-10 transition-transform transform hover:scale-110 z-50'
    onClick={()=>{
